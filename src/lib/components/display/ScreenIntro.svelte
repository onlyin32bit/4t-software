...
<div class="fixed h-full w-full bg-bg-3 bg-cover bg-no-repeat" in:scale={{ duration: 900 }}>
	<h1
		class="center-element fixed w-3/4 text-center font-game-display text-[18vh] font-bold"
		style={`-webkit-text-stroke: 1px #fff; color: #${nameMap.get(screen)?.color}; ${screen === 'vd' ? '-webkit-box-reflect: below -9vh linear-gradient(to bottom, rgba(0,0,0,0.0), rgba(0,0,0,0.4));' : ''}`}
