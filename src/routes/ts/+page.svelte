<script lang="ts">
		...
		unsub[1] = await pb
			...
			.subscribe('4T-DISPLAYSTATE', ({ action, record }) => {
				if (action === 'update') {
					...
					if (current.question !== record.ques) {
	...
	async function ringBell() {
		if (current.screen === 'vcnv' || bellAllowed)
			if (thisUser.ring === 0) {
				await pb.collection('users').update(thisUser.id, {
					ring: 1
				});
...
<div class="grid h-screen grid-cols-1 grid-rows-[50px_50px_1fr_120px] border-[3px] border-gray-400">
	...
	{#if (current.screen === 'kd' && current.slide === 'ques_chung') || current.screen === 'vd'}
		<button
	...
	{:else if current.screen === 'vcnv' || current.screen === 'tt'}
		<div class="grid grid-cols-5">
			<div class="col-span-4 grid grid-rows-[1fr_100px] text-6xl">
				...
				<div class="grid grid-cols-[1fr_12rem] grid-rows-2 border-[3px] border-gray-400">
